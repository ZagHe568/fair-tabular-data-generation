...
if __name__ == '__main__':
    ...
    df_train_data_no_gender = df_train_data.drop(columns='sex')
    df_gender = df_train_data['sex']
    ...
    print('Training....')
    ctgan.fit(df_train_data_no_gender, discrete_columns, epochs=1000)
    print('Done')

    df_generated = ctgan.sample(df_train_data.shape[0])

    from sklearn.linear_model import LogisticRegression
    from sklearn.preprocessing import LabelBinarizer, OneHotEncoder

    enc_relation = OneHotEncoder(handle_unknown='ignore')
    X = enc_relation.fit_transform(df_train_data['relationship'].values.reshape([-1, 1]).astype(str))
    enc_sex = LabelBinarizer()
    Y = enc_sex.fit_transform(df_train_data['sex'].values.reshape([-1, 1]))
    clf = LogisticRegression(random_state=0).fit(X, Y)
    X_generated = df_generated['relationship']
    X_generated = enc_relation.transform(X_generated.values.reshape([-1, 1]))
    Y_generated = clf.predict(X_generated)
    Y_generated = enc_sex.inverse_transform(Y_generated)
    df_generated['sex'] = Y_generated
    ...
    top_k_points = top_k_points[list(df_train_data.columns)]
